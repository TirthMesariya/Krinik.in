...
window.prev = prev;
  ...
  downloadBtn.addEventListener('click', () => {
    ...
    array.forEach((object, i) => {
        const formattedMobileNo = object.mobile_no ? `${object.mobile_no}` : "N/A"; // Ensure Excel treats it as text
        ...
        const row = `
            <tr>
               ...
               <td>${object["email"] || ""}</td>
               <td>${object["wallet_amount"] || 0}</td>
               <td>${object["winning_amount"] || 0}</td>
               <td>${object["status"] || 0}</td>
            </tr>`;
        tbody.innerHTML += row;
    });
    ...
    // Set Mobile Number Column as Text
    const range = XLSX.utils.decode_range(sheet['!ref']);
    for (let R = range.s.r + 1; R <= range.e.r; ++R) {
        const cellAddress = XLSX.utils.encode_cell({ r: R, c: 2 }); // Column index for "Mobile No."
        if (sheet[cellAddress]) {
            sheet[cellAddress].z = "@"; // Format as text
            sheet[cellAddress].t = "s"; // Ensure it's a string
        }
    }
    ...
    sheet['!cols'] = [
        ...
        { wch: 20 },  // Mobile No.
        { wch: 25 },  // Email
        { wch: 15 },  // Wallet Amount
        { wch: 15 },  // Winning Amount
        { wch: 15 },  // Status
    ];
